...
const FRIGORIFICOS_FALLBACK = [
  ...
  },
  {
    ...
    capacidade: 1100,
    status: "Ativo",
    descricao: "O Frigovale opera com uma filosofia pragmática: escala acima de tudo. O relacionamento com o pecuarista é direto, sem formalidades e focado em fechar negócio rapidamente.",
    pagamento: [
      "30/60 dias: pagamento dividido em duas parcelas iguais",
      "+3% sobre o valor total do lote (bônus pelo prazo longo)",
      "Risco de atraso: 25% de chance de atraso na segunda parcela"
    ]
  },
  {
    id: 4,
    ...
    capacidade: 1000,
    status: "Ativo",
    descricao: "Relacionamento com o Cliente: Este frigorífico busca produtores que trabalham com raças específicas e manejo diferenciado, visando um mercado de alto padrão.",
    pagamento: ["Pagamento em 7 dias", "Prêmio fixo por arroba até 20% acima do mercado"]
  }
];
...
function Frigorificos() {
  ...
  console.log('Frigoríficos carregados:', frigorificosData);
  ...
  useEffect(() => {
    try {
      const frigorificosImport = require('../data/frigorificos.json');
      setFrigorificosData(frigorificosImport);
      console.log('Frigoríficos carregados com sucesso:', frigorificosImport);
    } catch (error) {
  ...
  // Busca o valor da arroba (apenas manual por enquanto)
  useEffect(() => {
    async function fetchPreco() {
      setLoading(true);
      try {
        // Usa apenas o preço manual por enquanto
        const dataManual = await getArrobaManual();
        const valor = Number(dataManual.arroba) || 0;
        const fonte = 'Manual';
        
        console.log('Preço manual carregado:', valor);
        ...
        setPrecoArroba(283); // Fallback para R$ 283,00
      }
      setLoading(false);
    }
    fetchPreco();
  ...
  return (
    <div>
          ...
          <div style={{ background:'#23272f', borderRadius:12, padding:32, minWidth:320 }}>
            ...
            <div style={{display:'flex',gap:12,marginTop:24}}>
              ...
              <button onClick={async ()=>{
                try {
                  // Dados simulados para PDF
                  const pesoVivo = loteSelecionado.length * 500;
                  const pesoCarcaca = pesoVivo * 0.54;
                  const rendimento = (pesoCarcaca / pesoVivo) * 100;
                  const bonificacao = 0;
                  const valorBruto = (pesoCarcaca/15) * precoArroba;
                  const valorLiquido = valorBruto + bonificacao;
                  const dataVenda = new Date().toISOString().slice(0,10);
                  await registrarVendaFrigorifico({
                    frigorificoId: frigorificoEscolhido.id,
                  ...
                  setPdfData({
                    frigorifico: frigorificoEscolhido,
                    produtor: 'Jogador',
      ...
      {/* Botão de emissão de PDF após venda */}
      {pdfData && (
        <div style={{ position: 'fixed', top:0, left:0, width:'100vw', height:'100vh', background:'#0008', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#23272f', borderRadius:12, padding:32, minWidth:320, textAlign:'center' }}>
            <h3>Venda registrada!</h3>
            <p style={{color:'#b0bec5'}}>Clique abaixo para baixar o PDF do romaneio do lote.</p>
            <button style={{ padding: '12px 28px', borderRadius: 8, background: '#1976d2', color: '#fff', border: 'none', fontWeight: 600, fontSize: 17, cursor: 'pointer', marginTop: 16 }}
              onClick={()=>{
                emitirRomaneioPDF(pdfData);
                setPdfData(null);
              }}>
              Baixar PDF do Romaneio
            </button>
            <div>
              <button style={{marginTop:16}} onClick={()=>setPdfData(null)}>Fechar</button>
            </div>
          </div>
        </div>
      )}
